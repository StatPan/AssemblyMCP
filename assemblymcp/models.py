...
class Bill(BaseModel):
    ...
    proposer: str = Field(..., description="대표 제안자 (PROPOSER). 의안을 대표로 발의한 국회의원 또는 위원회 이름.")
    proposer_text: str | None = Field(None, description="원본 제안자 문자열 (예: '홍길동의원 등 10인').")
    primary_proposer: str | None = Field(None, description="대표 제안자 이름만 추출한 값.")
    proposer_count: int | None = Field(None, description="총 제안자 수 (파생 정보, 추정값일 수 있음).")
    proposer_kind_name: str = Field(..., description="제안자 구분 (PROPOSER_KIND). '의원' 또는 '위원회'.")
    proc_status: str = Field(..., description="처리상태 (PROC_STATUS). 의안의 현재 처리 단계 (예: 위원회 심사 등).")
    committee: str = Field(..., description="소관위원회 (COMMITTEE). 의안이 소관된 위원회의 이름.")
    propose_dt: str | None = Field(None, description="제안일자 (PROPOSE_DT). 의안이 발의된 날짜 (YYYY-MM-DD 형식).")
    committee_dt: str | None = Field(
        None, description="회부일자 (COMMITTEE_DT). 위원회에 회부된 날짜 (YYYY-MM-DD 형식)."
    )
    proc_dt: str | None = Field(None, description="최종 처리일자 (PROC_DT). 의안이 최종 처리된 날짜 (YYYY-MM-DD 형식).")
...
class BillDetail(Bill):
    ...
    summary: str | None = Field(None, description="주요내용 (MAJOR_CONTENT). 법률안의 주요 내용 요약.")
...
class Committee(BaseModel):
    ...
    committee_div: str | None = Field(None, description="위원회 구분 (CMT_DIV_NM). 예: 상임위원회, 특별위원회.")
