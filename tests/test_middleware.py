...
from assemblymcp.middleware import LoggingMiddleware, CachingMiddleware
from assemblymcp.config import settings

# Mock CallToolRequest
def create_mock_context(tool_name="test_tool", arguments={"arg": "value"}):
...
async def test_logging_middleware(caplog):
    ...
    # No, our formatter is attached to handler, caplog might capture before formatting or use its own handler.
...
async def test_caching_middleware():
    ...
    assert getattr(result2, "_is_cached", False) == True
